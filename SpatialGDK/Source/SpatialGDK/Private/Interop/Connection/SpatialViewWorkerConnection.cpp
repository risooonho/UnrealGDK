...
#include "SpatialView/ConnectionHandlers/SpatialOSConnectionHandler.h"
...
TArray<SpatialGDK::OpList> USpatialViewWorkerConnection::GetOpList()
{
	...
	OpLists.Add(Coordinator->GetLegacyOpList());
	return OpLists;
}
...
void USpatialViewWorkerConnection::SendMetrics(const SpatialGDK::SpatialMetrics& Metrics)
{
	check(Coordinator.IsValid());
	// todo implement
}
