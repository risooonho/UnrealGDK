...
bool USpatialSender::WillHaveAuthorityOverActor(AActor* TargetActor, Worker_EntityId TargetEntity) const
{
	...
	if (GetDefault<USpatialGDKSettings>()->bEnableUnrealLoadBalancer)
	{
		...
		{
			if (const SpatialGDK::AuthorityDelegation* AuthorityDelegationComponent = StaticComponentView->GetComponentData<SpatialGDK::AuthorityDelegation>(TargetEntity))
			{
				const Worker_PartitionId IntendedPartitionId = *AuthorityDelegationComponent->Delegations.Find(SpatialConstants::POSITION_COMPONENT_ID);
				if (IntendedPartitionId != NetDriver->VirtualWorkerTranslator->GetClaimedPartitionId())
				{
					WillHaveAuthorityOverActor = false;
				}
			}
